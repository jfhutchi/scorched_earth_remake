...
- Implemented the v0.9.2 Castle Siege Armory: spends earned siege coins on one-attempt bonus ammo caches (Precision, Heavy, Excavator, Cluster), accessible from level select and the result overlay, with stock persistence under the existing `crater-command-siege-progress-v1` localStorage key and backward-compatible `armory: {}` shape.
